...
int main(int argc, char const *argv[])
{
    ...
    while (1)
    {
        //borramos ambos buffers para evitar
        ...
        msg.data_length = read(1, &msg.data, MAX_DATA_LENGTH);
...
{
    int res = 0, ret_wr = 0;
    char bufi[MAX_MSG_LENGTH];
    memset(bufi, 0, sizeof(bufi)); //borramos el buffer
    ...
    if (msg.message_type == START_MSG)
    {
        ...
        sprintf(bufi, "%4d%4d%4d", msg.message_length, msg.message_type, msg.PID);
    ...
    {
        sprintf(bufi, "%4d%4d", msg.message_length, msg.message_type);
        ret_wr = write(fd, &bufi, msg.message_length);
        return ret_wr;
    }
